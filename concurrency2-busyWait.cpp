...
public:
    ...
    void operator()(int& x) {
        while (true) {
            while (turn!=0); // loop and wait till Critical section execution
            x += 1;
...
public:
    void non_critical_section() {
        int a = 0;
        while (++a < 100000000); // slows the speed of  execution of process 1 non_critical_section
    }

    void operator()(int& x) {
        while(true) {
            while (turn != 1); // wait till turn of process 1
            x += 1;
